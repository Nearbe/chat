...
- [x] **Добавление Unit и UI тестов**
    ...
    - [x] Запустить `deploy.sh` для перегенерации проекта Xcode.
...
- [ ] **Оптимизация поиска и работы с данными**
    - [ ] Рефакторинг `HistoryView`: заменить фильтрацию массива на использование `#Predicate` в `@Query` (динамическая фильтрация).
    - [ ] Реализовать механизмы пагинации (fetch limit/offset) для больших списков сообщений.
    - [ ] Настроить индексы в моделях SwiftData для ускорения выборок по дате и отправителю.
