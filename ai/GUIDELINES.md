...
- Используйте модификаторы из `Typography.swift` и `Spacing.swift` для единообразия.<document>
  <metadata>
...
**Сетевой слой**:
- Реализован через `HTTPClient` и `NetworkService`
...
**Авторизация**:
- Привязана к устройству через `DeviceConfiguration` и `DeviceIdentity`
...
Для быстрого погружения в проект используйте эти файлы:

| Компонент | Путь |
|-----------|------|
| **Жизненный цикл приложения** | [`App/ChatApp.swift`](../../App/ChatApp.swift) |
| **Главный экран (View)** | [`Features/Chat/Views/ChatView.swift`](../../Features/Chat/Views/ChatView.swift) |
| **Основная логика (ViewModel)** | [`Features/Chat/ViewModels/ChatViewModel.swift`](../../Features/Chat/ViewModels/ChatViewModel.swift) |
| **Сетевой шлюз** | [`Services/Network/NetworkService.swift`](../../Services/Network/NetworkService.swift) |
| **Персистентность данных (SwiftData)** | [`Data/PersistenceController.swift`](../../Data/PersistenceController.swift) |
| **Каталог агентов** | [`Agents/`](../../Agents/) |
    </section>
...
- [migrations/junie_migration.md](./migrations/junie_migration.md) — архив информации из .junie/
...
- Избегайте "магических чисел" в отступах и цветах — используйте модификаторы из `Typography.swift` и `Spacing.swift` для единообразия.
- Секретные данные (токены) хранятся **только** в Keychain через `KeychainHelper`. Не сохраняйте секреты в `UserDefaults`.
    </section>
...
1. **Обновление структуры папок или ключевых файлов**:
    ...
    - Обновите таблицу с ключевыми точками входа (`key_entry_points`) в этом же файле.
2. **Изменение архитектурных подходов**:
    - Если в проект внедряется новая технология или меняется способ работы с существующей (например, замена SwiftData на CoreData), обновите `GUIDELINES.md`.
...
4. **Синхронизация документации**:
    - После завершения работы проверьте, что `GUIDELINES.md`, `QWEN.md`, `SETUP.md` и `TESTING.md` согласованы между собой.
...
Автоматизация (`check.sh`) настроена на использование симуляторов без подписания кода, чтобы избежать запросов пароля.
    </section>
