...
1. **Генерация и сборка проекта**:
   Проект должен собираться для реального устройства **iPhone 16 Pro Max** с именем **Saint Celestine**. Для этого используйте скрипт деплоя, который автоматически сгенерирует `.xcodeproj` и соберет приложение:
   ```bash
   chmod +x deploy.sh
   ./deploy.sh
   ```

2. **Ручная сборка и тестирование**:
   Любые сборки и запуски тестов ДОЛЖНЫ выполняться на устройстве **Saint Celestine** (iPhone 16 Pro Max), которое всегда подключено к макбуку.
   ```bash
   # Сборка
   xcodebuild -project Chat.xcodeproj -scheme Chat -configuration Debug -destination "platform=iOS,name=Saint Celestine" build
   
   # Запуск тестов
   xcodebuild -project Chat.xcodeproj -scheme Chat -destination "platform=iOS,name=Saint Celestine" test
   ```
...
2. **Инструменты**:
   - Для деплоя на устройство (**Saint Celestine**) используется `deploy.sh`. Он автоматически запускает XcodeGen для генерации проекта.
