...
3. `QWEN.md` — полный контекст проекта для AI-агентов.
4. `AGENTS.md` — краткое руководство для AI-агентов.
5. `.junie/instructions.md` — этот файл.
6. `SETUP.md` — настройка окружения.
7. `TESTING.md` — руководство по тестированию.
8. `VERSIONING.md` — политика версионирования.
...
Для оценки состояния проекта используется навык `project_analysis`. Доступны режимы:

| Режим           | Флаг                   | Описание                      |
|-----------------|------------------------|-------------------------------|
| Инкрементальный | (по умолч.)            | Только изменённые зоны по git |
| Полный          | `--full`               | Все роли, переиспользует кэш  |
...
project_analysis --review-role client_developer
```
...
**Важно:** Приоритеты основаны на математических критериях, а не на времени. Время (недели, спринты) не используется.

### Определение приоритетов

| Приоритет | Критерий                           | Описание                   |
|-----------|------------------------------------|----------------------------|
| **P0**    | Блокирует >5 задач ИЛИ Риск ≥ 80   | Критическая проблема       |
| **P1**    | Блокирует 2-5 задач ИЛИ Риск 40-80 | Высокий приоритет          |
| **P2**    | Блокирует 1 задачу ИЛИ Риск < 40   | Средний приоритет          |
| **P3**    | Нет блокировок, Риск < 30          | Низкий приоритет (roadmap) |
