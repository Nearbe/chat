...
public final class DatabaseManager: @unchecked Sendable {
    ...
    private func createTable() throws {
        ...
        try db.run(metrics.create(ifNotExists: true) {
            t in
    ...
    public func save(_ record: MetricsRecord) {
        guard let db = db else {
            return
        }
    ...
    public func fetchAll() -> [MetricsRecord] {
        ...
        do {
            return try db.prepare(metrics.order(colTimestamp.desc)).map {
                row in
                mapRowToRecord(row)
            }
        } catch {
    ...
    public func fetch(byOperation operation: String, limit: Int = 10) -> [MetricsRecord] {
        ...
        do {
            let query = metrics.filter(colOperation == operation).order(colTimestamp.desc).limit(limit)
    ...
    func fetchLast() -> MetricsRecord? {
        ...
        do {
            ...
            return try db.prepare(query).compactMap {
                mapRowToRecord($0)
            }.first
        } catch {
            return nil
