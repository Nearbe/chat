...
- `Tools/Scripts/`: Скрипты автоматизации проекта на Swift (XcodeGen, SwiftGen, Check, Ship и др.). Команды запускаются через `swift run scripts <command>`.
...
Основные команды:
- `swift run scripts setup` — Подготовка проекта (XcodeGen + SwiftGen).
- `swift run scripts check` — Полная техническая проверка (выполняется асинхронно для ускорения).
- `swift run scripts ship` — Деплой на устройство (требует настройки sudo).
- `swift run scripts download-docs` — Обновление документации инструментов.
...
Для обновления этих данных используйте `swift run scripts download-docs`.
...
5. **Проверки качества и коммит**:
   - Перед отправкой изменений ОБЯЗАТЕЛЬНО выполните `swift run scripts check`.
   - Если требуется доставить продукт для проверки, выполните `swift run scripts ship` ПОСЛЕ `swift run scripts check`.
   - Что делает `swift run scripts check`:
