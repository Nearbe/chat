...
- `check.sh`: Техническая проверка (Lint + Build + Test + Commit) для разработчика/ИИ.
- `ship.sh`: Доставка продукта (Release Build + Deploy) для главного потребителя. Должен запускаться ПОСЛЕ `check.sh`.
