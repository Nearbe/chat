...
## Настройка беспарольного запуска (Sudoers)
Чтобы `ship.sh` никогда не запрашивал пароль при деплое на реальное устройство, его нужно добавить в исключения `sudoers`. 
Это безопасно, так как исключение выдается на **конкретный скрипт** в конкретной папке.

1. Запустите вспомогательный скрипт:
   ```bash
   ./scripts/configure_sudo.sh
   ```
2. Скопируйте предложенную команду и выполните её. Обычно она выглядит так:
   ```bash
