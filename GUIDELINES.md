...
В IntelliJ IDEA эти схемы автоматически подхватываются и отображаются в меню Run Configuration. Прямая поддержка в `project.yml` гарантирует сохранение настроек при перегенерации проекта.
...
5. **Проверки качества и коммит**:
   ...
   - Что делает `./scripts check` (выполняется максимально асинхронно):
     ...
     - **`ProjectChecker`** (специальные проверки проекта):
       - Наличие документации (Docstrings) для всех типов и методов.
       - Использование **русского языка** в документации.
       - Запрет на использование `print()` (используйте Pulse или систему логирования).
       - Обязательное использование `@MainActor` для всех классов `ViewModel`.
